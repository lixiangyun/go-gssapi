...
func verifyInquireContextResult(result string, regexps []string) {
	rr := strings.Split(result, " ")
	if len(rr) != len(regexps) {
		log.Fatalf("got %v fragments, expected %v (%s)", len(rr), len(regexps), result)
	}

	for i, r := range rr {
		rx := regexp.MustCompile(regexps[i])
		if !rx.MatchString(r) {
			log.Errorf("%s does not match %s", r, regexps[i])
		}
	}
}

func TestClientInquireContext() {

	ctx, r := initClientContext("GET", "/inquire_context/", nil)
	defer ctx.Release()
