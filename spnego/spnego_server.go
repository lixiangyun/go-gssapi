...
func (k KerberizedServer) AcquireCred(serviceName string) (*gssapi.CredId, error) {
	...
	cred, actualMechs, _, err := gssapi.AcquireCred(name, gssapi.GSS_C_INDEFINITE, gssapi.GSS_C_NO_OID_SET, gssapi.GSS_C_ACCEPT)
	if err != nil {
		return nil, err
	}
...
func (k KerberizedServer) Negotiate(cred *gssapi.CredId, inHeader, outHeader http.Header) (string, int, error) {
	negotiate, inputToken := CheckSPNEGONegotiate(inHeader, "Authorization")
	...
	ctx, srcName, _, outputToken, _, _, delegatedCredHandle, err :=
		gssapi.AcceptSecContext(gssapi.GSS_C_NO_CONTEXT,
